	...
	public static class WavUtility
	{
		...
		private static float[] Convert8BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
		{
			...
			if (!(wavSize > 0 && wavSize == dataSize))
				throw new FormatException($"Failed to get valid 8-bit wav size: {wavSize} from data bytes: {dataSize} at offset: {headerOffset}");
		...
		private static float[] Convert24BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
		{
			...
			headerOffset += sizeof(int);
			if (!(wavSize > 0 && wavSize == dataSize))
				throw new FormatException($"Failed to get valid 24-bit wav size: {wavSize} from data bytes: {dataSize} at offset: {headerOffset}");
		...
		private static float[] Convert32BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
		{
			...
			headerOffset += sizeof(int);
			if (!(wavSize > 0 && wavSize == dataSize))
				throw new FormatException($"Failed to get valid 32-bit wav size: {wavSize} from data bytes: {dataSize} at offset: {headerOffset}");
